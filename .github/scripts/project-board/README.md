...
- **Dry run:** leave the source-controlled `DRY_RUN` constant in `src/main.ts` set to `true`. Exercise representative open, ready, draft, review-request, review, dismissal, and reopen events. Confirm the logs show the expected lane without changing Status. Confirm the scheduled Lodestar sweep covers cards from every participating repository.
- **Enable writes:** set the central `DRY_RUN` constant to `false` and merge it to the protected `unstable` branch. This one setting applies to Lodestar and every caller repository. Exercise one representative PR before broad rollout and confirm both event-driven updates and the scheduled sweep.
...
- **Rollback:** restore the central `DRY_RUN` constant to `true` or disable the caller workflow in affected repositories. Leave native auto-add, close, merge, and archive workflows enabled so project membership and terminal states continue to work. Re-enable the reconciler and run a sweep after correcting the issue.
