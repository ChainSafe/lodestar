...
- Auth: the default `GITHUB_TOKEN` cannot access org projects (documented). Interim: fine-grained PAT (resource owner ChainSafe; org **Projects: read/write** + **Members: read**, repo **Pull requests: read** + **Metadata: read**) stored as the `PROJECT_BOARD_TOKEN` repository secret in Lodestar and every caller repository. Target: an org-owned GitHub App; swapping replaces the secret with an `actions/create-github-app-token` step and leaves the logic unchanged. The workflow job skips repositories outside the ChainSafe organization, and the script tolerates a missing token when the workflow repository owner is outside `PROJECT_ORG`. A missing token is also tolerated when the event payload confirms an external-fork review or a Dependabot-triggered PR event; every other context fails with `PROJECT_BOARD_CONFIG_TOKEN_MISSING`.
