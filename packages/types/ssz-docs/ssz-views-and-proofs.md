...
Think of **views** as _live, Merkle-aware wrappers around SSZ data_.
...
- Views (`defaultView()`) give you an _optimized Merkle tree representation_ that allows:
...
- 'defaultView()' -> This creates a tree backed object.
...
Both are wrappers around a Merkle tree + SSZ type schema: they expose an object-like API for convenient property access.
...
Property getters return sub-views, except for basic types, which return native values. Setters, likewise, require sub-views, except for basic types, which require native values.
...
View implementations don't contain any internal caches beyond their internal Trees, and setting one _subview_ to another will not link the views.
...
2. **TreeViewDu**
   ViewDU = View Deferred Update. This tree view caches all mutations to data and applies the changes to the tree only when you call the commit() method. This allows you to pay the cost of navigating and updating the tree only once.This strategy is optimal for large tree manipulations that require very high performance (i.e. the Ethereum consensus beacon chain state transition).
...
Mutations are staged in memory — temporarily cached — until you commit them.
...
How to actually work with views in everyday SSZ use — things like serializing, hashing, cloning, committing, etc.
...
The formula is:
gindex = 2^depth + index at that depth
Let's visualize:
...
Below are some common pitfalls and how to avoid them when working with Lodestar’s SSZ Views and Proofs.
