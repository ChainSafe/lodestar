...
import {StringType} from "../../primitive/string";
...
import {IPhase0SSZTypesOnly} from "./interface";

export function getPhase0Types(params: IPhase0Params, primitive: IPrimitiveSSZTypes): IPhase0SSZTypesOnly {
  const {
    BLSSignature,
    BLSPubkey,
    Version,
  ...
  const typesRef = new LazyVariable<IPhase0SSZTypesOnly>();
  ...
  const phase0Types: IPhase0SSZTypesOnly = {
    // misc
    Fork,
    ForkData,
    ENRForkID,
