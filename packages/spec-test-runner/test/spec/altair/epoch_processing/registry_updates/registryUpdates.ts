...
export function runRegistryUpdates(presetName: PresetName): void {
  ...
  describeDirectorySpecTest<IAltairStateTestCase, altairTypes.BeaconState>(
    `altair epoch registry updates ${presetName}`,
    join(SPEC_TEST_LOCATION, `tests/${presetName}/altair/epoch_processing/registry_updates/pyspec_tests`),
    (testcase) => {
