...
export function runHistoricalRootsUpdate(presetName: PresetName): void {
  ...
  describeDirectorySpecTest<IAltairStateTestCase, altairTypes.BeaconState>(
    `altair epoch historical roots update ${presetName}`,
    join(SPEC_TEST_LOCATION, `tests/${presetName}/altair/epoch_processing/historical_roots_update/pyspec_tests`),
    (testcase) => {
