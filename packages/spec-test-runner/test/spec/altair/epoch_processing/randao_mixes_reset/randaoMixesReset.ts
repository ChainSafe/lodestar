...
export function runRandaoMixesReset(presetName: PresetName): void {
  ...
  describeDirectorySpecTest<IAltairStateTestCase, altairTypes.BeaconState>(
    `altair epoch randao mixes reset ${presetName}`,
    join(SPEC_TEST_LOCATION, `tests/${presetName}/altair/epoch_processing/randao_mixes_reset/pyspec_tests`),
    (testcase) => {
