...
export function runEffectiveBalanceUpdates(presetName: PresetName): void {
  ...
  describeDirectorySpecTest<IAltairStateTestCase, altairTypes.BeaconState>(
    `altair epoch effective balance updates ${presetName}`,
    join(SPEC_TEST_LOCATION, `tests/${presetName}/altair/epoch_processing/effective_balance_updates/pyspec_tests`),
    (testcase) => {
