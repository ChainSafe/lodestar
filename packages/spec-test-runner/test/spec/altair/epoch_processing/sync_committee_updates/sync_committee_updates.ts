...
export function runSyncCommitteeUpdates(presetName: PresetName): void {
  ...
  describeDirectorySpecTest<IAltairStateTestCase, altairTypes.BeaconState>(
    `altair epoch sync committee updates ${presetName}`,
    join(SPEC_TEST_LOCATION, `tests/${presetName}/altair/epoch_processing/sync_committee_updates/pyspec_tests`),
    (testcase) => {
