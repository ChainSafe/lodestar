...
export function runInactivityUpdates(presetName: PresetName): void {
  ...
  describeDirectorySpecTest<IAltairStateTestCase, altairTypes.BeaconState>(
    `altair process inactivity updates ${presetName}`,
    join(SPEC_TEST_LOCATION, `tests/${presetName}/altair/epoch_processing/inactivity_updates/pyspec_tests`),
    (testcase) => {
