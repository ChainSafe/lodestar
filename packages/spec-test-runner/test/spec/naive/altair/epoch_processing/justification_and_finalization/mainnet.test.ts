...
describeDirectorySpecTest<IAltairStateTestCase, altairTypes.BeaconState>(
  ...
  join(SPEC_TEST_LOCATION, "tests/mainnet/altair/epoch_processing/justification_and_finalization/pyspec_tests"),
  (testcase) => {
    const state = testcase.pre;
    naive.altair.processJustificationAndFinalization(config, state);
    return state;
  },
  {
    inputTypes: {
