...
describeDirectorySpecTest<IStateTestCase, phase0.BeaconState>(
  "eth1 data reset minimal",
  join(SPEC_TEST_LOCATION, "tests/minimal/phase0/epoch_processing/eth1_data_reset/pyspec_tests"),
  (testcase) => {
    const state = testcase.pre;
    naive.phase0.processEth1DataReset(config, state);
    return state;
  },
  {
    inputTypes: {
