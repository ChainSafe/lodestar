...
describeDirectorySpecTest<IStateTestCase, phase0.BeaconState>(
  "epoch effective balance updates mainnet",
  join(SPEC_TEST_LOCATION, "tests/mainnet/phase0/epoch_processing/effective_balance_updates/pyspec_tests"),
  (testcase) => {
    const state = testcase.pre;
    naive.phase0.processEffectiveBalanceUpdates(config, state);
    return state;
  },
  {
    inputTypes: {
