...
describeDirectorySpecTest<IStateTestCase, phase0.BeaconState>(
  "process randao mixes reset mainnet",
  join(SPEC_TEST_LOCATION, "tests/mainnet/phase0/epoch_processing/randao_mixes_reset/pyspec_tests"),
  (testcase) => {
    const state = testcase.pre;
    naive.phase0.processRandaoMixesReset(config, state);
    return state;
  },
  {
    inputTypes: {
