...
describeDirectorySpecTest<IStateTestCase, phase0.BeaconState>(
  "process historical roots updates minimal",
  join(SPEC_TEST_LOCATION, "tests/minimal/phase0/epoch_processing/historical_roots_update/pyspec_tests"),
  (testcase) => {
    const state = testcase.pre;
    naive.phase0.processHistoricalRootsUpdate(config, state);
    return state;
  },
  {
    inputTypes: {
