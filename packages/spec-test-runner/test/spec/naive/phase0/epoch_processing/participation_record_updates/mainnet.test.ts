...
describeDirectorySpecTest<IStateTestCase, phase0.BeaconState>(
  "process participation record updates mainnet",
  join(SPEC_TEST_LOCATION, "tests/mainnet/phase0/epoch_processing/participation_record_updates/pyspec_tests"),
  (testcase) => {
    const state = testcase.pre;
    naive.phase0.processParticipationRecordUpdates(config, state);
    return state;
  },
  {
    inputTypes: {
