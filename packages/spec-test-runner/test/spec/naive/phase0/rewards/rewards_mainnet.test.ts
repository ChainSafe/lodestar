...
for (const testSuite of ["basic", "leak", "random"]) {
  describeDirectorySpecTest<IRewardsTestCase, IAttestationDeltas>(
    ...
    (testcase) => {
      ...
      const sourceDeltas = naive.phase0.getSourceDeltas(config, state);
      const targetDeltas = naive.phase0.getTargetDeltas(config, state);
      const headDeltas = naive.phase0.getHeadDeltas(config, state);
      const inclusionDelayDeltas = naive.phase0.getInclusionDelayDeltas(config, state);
      const inactivityPenaltyDeltas = naive.phase0.getInactivityPenaltyDeltas(config, state);
      return {
