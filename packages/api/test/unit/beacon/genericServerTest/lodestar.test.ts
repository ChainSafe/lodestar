...
describe("beacon / lodestar", () => {
  ...
  describe("getMonitoredValidatorIndices", () => {
    ...
    it("should return monitored validator indices", async () => {
      const expectedIndices = [0, 1, 2, 5, 10, 100];
      mockApi.getMonitoredValidatorIndices.mockResolvedValue({
        data: expectedIndices,
      });

      const httpClient = new HttpClient({baseUrl});
      const client = getClient(config, httpClient);

      const res = await client.getMonitoredValidatorIndices();

      expect(res.ok).toBe(true);
      expect(res.value()).toEqual(expectedIndices);
    });

    it("should return empty array when no validators monitored", async () => {
      mockApi.getMonitoredValidatorIndices.mockResolvedValue({
        data: [],
      });
