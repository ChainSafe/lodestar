...
function getGethBinaryExecutionClient(): {
  availability: () => Promise<boolean>;
  setup: () => Promise<AbortController>;
  ...
  const availability = async (): Promise<boolean> => {
    return process.env.GETH_BINARY_PATH !== undefined;
  };

  const setup = async (): Promise<AbortController> => {
  ...
  return {availability, setup, teardown, apiUrl: `http://localhost:${enginePort}`, name: "geth-binary"};
}
...
function getGethDockerizedExecutionClient(): {
  ...
  const availability = async (): Promise<boolean> => {
    return process.env.GETH_DOCKER_COMPOSE_PATH !== undefined;
  };

  const setup = async (): Promise<AbortController> => {
    controller = new AbortController();
    shell(`cd ${process.env.GETH_DOCKER_COMPOSE_PATH} && docker-compose -f docker-compose.geth.yml up`, {
      timeout: 10 * 60 * 1000,
      signal: controller.signal,
    }).catch((e) => {
      gethProcError = e as Error;
  ...
  return {availability, setup, teardown, apiUrl: `http://localhost:${enginePort}`, name: "geth-dockerized"};
}

[getGethBinaryExecutionClient(), getGethDockerizedExecutionClient()].map(
  ({availability, setup, teardown, apiUrl, name}) => {
    describe(`executionEngine / ExecutionEngineHttp - ${name}`, function () {
      ...
      before(`Run ${name}`, async () => {
        ...
        if (clientAvailable) {
          ...
          for (let i = 0; i < 60; i++) {
            try {
              await shell(
                `curl -X POST -H "Content-Type: application/json" --data '{"jsonrpc":"2.0","method":"net_version","params":[],"id":67}' ${apiUrl}`
              );
              return; // Done
            } catch (e) {
          ...
          throw Error(`${name} not online in 60 seconds`);
        }
      });
      ...
      it(`Test interop apis with ${name}`, async () => {
        if (!clientAvailable) throw new Error("clientNotAvailable");

        await testExecutionClient(apiUrl);
      });

      after(`Stop ${name}`, async () => {
        if (clientAvailable) {
          await teardown();
        }
      });
...
async function testExecutionClient(gethUrl: string): Promise<void> {
