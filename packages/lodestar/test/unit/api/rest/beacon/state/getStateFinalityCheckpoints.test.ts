...
import {getBlock, getBlockRoot} from "../../../../../../src/api/rest/controllers/beacon/blocks";
...
describe("rest - beacon - getStateFinalityCheckpoints", function () {
    ...
    it("should succeed", async function () {
        ...
        const response = await supertest(restApi.server.server)
            .get(
                urlJoin(BEACON_PREFIX, getStateFinalityCheckpoints.url.replace(":stateId", "head"))
            )
            .expect(200)
    ...
    it("should not found state", async function () {
        ...
        await supertest(restApi.server.server)
            .get(urlJoin(BEACON_PREFIX, getBlock.url.replace(":stateId", "4")))
            .expect(404);
    });
});
