...
describe("api - beacon - getBlockHeader", function () {
  ...
  it("block not found", async function () {
    ...
    const result = await this.test?.ctx?.blockApi.getBlockHeader("1");
  ...
  it("invalid block id", async function () {
    ...
    await expect(this.test?.ctx?.blockApi.getBlockHeader("abc")).to.eventually.be.rejected;
  });
  ...
  it("success for block", async function () {
    ...
    const result = await this.test?.ctx?.blockApi.getBlockHeader("head");
