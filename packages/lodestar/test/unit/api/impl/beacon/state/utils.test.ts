...
describe("beacon state api utils", function () {
  describe("resolve state id", function () {
    ...
    it("resolve head state id - success", async function () {
      ...
      const state = await resolveStateId(chainStub, this.test?.ctx?.dbStub, "head");
    ...
    it("resolve genesis state id - success", async function () {
      this.test?.ctx?.dbStub.stateArchive.get.withArgs(0).resolves(generateState());
      const state = await resolveStateId(chainStub, this.test?.ctx?.dbStub, "genesis");
      expect(state).to.not.be.null;
    ...
    it("resolve state by root", async function () {
      chainStub.stateCache.get.returns({state: generateCachedState(), epochCtx});
      const state = await resolveStateId(chainStub, this.test?.ctx?.dbStub, toHexString(Buffer.alloc(32, 1)));
      expect(state).to.not.be.null;
      expect(chainStub.stateCache.get.calledOnce).to.be.true;
    });
    ...
    it("state id is invalid root", async function () {
      await expect(resolveStateId(chainStub, this.test?.ctx?.dbStub, "adcas")).to.be.eventually.rejected;
    ...
    it("resolve state by slot", async function () {
      ...
      chainStub.stateCache.get.returns({state: generateCachedState(), epochCtx});
      const state = await resolveStateId(chainStub, this.test?.ctx?.dbStub, "123");
      expect(state).to.not.be.null;
