...
export class InteropSubnetsJoiningTask implements ITask {
  ...
  private subscribeToRandomSubnet(): number {
    ...
    setTimeout(
      ...
      this.config.params.EPOCHS_PER_RANDOM_SUBNET_SUBSCRIPTION * this.config.params.SLOTS_PER_EPOCH * this.config.params.SECONDS_PER_SLOT * 1000,
      subnet
    );
    return subnet;
  }

  private handleChangeSubnets = (subnet: number) => {
