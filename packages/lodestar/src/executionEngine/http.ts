...
export class ExecutionEngineHttp implements IExecutionEngine {
  ...
  async executePayload(
    executionPayload: merge.ExecutionPayload,
    forkChoice?: IForkChoice
  ): Promise<ExecutePayloadStatus> {
    ...
    // Validate blocks in forkchoice till latestValidHash, Invalidate decendants  if response status is invalid.
    if (forkChoice) {
      forkChoice?.validateLatestHash(
        latestValidHash,
        statusEnum === ExecutePayloadStatus.INVALID
          ? latestValidHash === serializedExecutionPayload.parentHash
            ? serializedExecutionPayload.blockHash
            : serializedExecutionPayload.parentHash
          : null
      );
    } else {
      // If forChoice not provided, fail hard on INVALID status as this is the scenario where the payload was generated by this EL itself and executePayload call here is in "fire and forget" mode for EL
      // TODO: throwing error is not failing hard enough, decide and implement switch over to a different available EL or node failure
      if (statusEnum == ExecutePayloadStatus.INVALID) throw Error("InconsistentELOnExecutePayload");
    }
