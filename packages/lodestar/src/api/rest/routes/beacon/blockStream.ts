...
export const registerBlockStreamEndpoint: LodestarRestApiEndpoint = (server, {api, config}): void => {
  server.get<fastify.DefaultQuery, {}, unknown>("/blocks/stream", {}, async (request, reply) => {
    ...
    const transform = (source: AsyncIterable<SignedBeaconBlock>): AsyncIterable<EventMessage> =>
      (async function* () {
