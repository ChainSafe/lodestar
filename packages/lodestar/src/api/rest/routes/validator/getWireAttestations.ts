...
export const registerGetWireAttestationEndpoint: LodestarRestApiEndpoint = (fastify, {api, config}): void => {
  fastify.get<IQuery>("/wire_attestations", opts, async (request, reply) => {
    const responseValue = await api.validator.getWireAttestations(request.query.epoch, request.query.committee_index);
    reply
      .code(200)
