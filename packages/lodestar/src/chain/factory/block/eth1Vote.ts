...
export async function getEth1Vote(
  ...
  const validVotes = Array.from(state.eth1DataVotes).filter(
    (eth1Data) => votesToConsider[toHexString(eth1Data.blockHash)]
  );
  ...
  if (validVotes.length > 0) {
    ...
    const defaultVote: Eth1Data = validEth1Data.length > 0 ? validEth1Data[validEth1Data.length - 1] : state.eth1Data;
    return defaultVote;
  }
}
