...
export async function processAttestation(
  ...
  logger: ILogger, db: IBeaconDb,
  attestation: Attestation): Promise<void> {
  ...
  try {
    ...
    if(!isValidIndexedAttestation(
      attestationPreState.epochCtx, attestationPreState.state, indexedAttestation, true
    )) {
      logger.verbose(
        "Dropping attestation from processing",
