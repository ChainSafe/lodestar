...
tmux new-session -d -s interop "./bin/lodestar interop -p minimal --db l0 -q $START_TIME,$TOTAL_VALIDATORS --localMultiaddrs /ip4/127.0.0.1/tcp/${BASE_PORT}0 -v ${VALIDATOR_ARRAY[0]} -r";
for (( i=1; i<$nodes; i++ )) do
    tmux split-window -v -t interop "./bin/lodestar interop -p minimal --db l$i -q $START_TIME,$TOTAL_VALIDATORS --localMultiaddrs /ip4/127.0.0.1/tcp/$BASE_PORT$i -v ${VALIDATOR_ARRAY[$i-1]},${VALIDATOR_ARRAY[$i]} -r"
done
