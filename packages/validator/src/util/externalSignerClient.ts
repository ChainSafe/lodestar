...
export type SignableMessage =
  ...
  | {type: SignableMessageType.REQUEST_AUTH; data: gloas.RequestAuthV1};
...
const requiresForkInfo: Record<SignableMessageType, boolean> = {
  ...
  // Fork-independent domain: compute_domain(DOMAIN_REQUEST_AUTH) with genesis fork version and zero genesis validators root
...
function serializerSignableMessagePayload(config: BeaconConfig, payload: SignableMessage): Record<string, unknown> {
  switch (payload.type) {
    ...
    case SignableMessageType.REQUEST_AUTH:
      return {request_auth: ssz.gloas.RequestAuthV1.toJson(payload.data)};
  }
}
