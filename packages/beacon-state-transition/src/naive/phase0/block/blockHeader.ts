...
export function processBlockHeader(config: IBeaconConfig, state: phase0.BeaconState, block: phase0.BeaconBlock): void {
  ...
  assert.equal(block.proposerIndex, getBeaconProposerIndex(state), "Incorrect proposer index");
  // Verify that the parent matches
  assert.true(
    ssz.Root.equals(block.parentRoot, ssz.phase0.BeaconBlockHeader.hashTreeRoot(state.latestBlockHeader)),
