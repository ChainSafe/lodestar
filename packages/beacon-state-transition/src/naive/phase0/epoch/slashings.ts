 ...
 */
export function processSlashings(state: phase0.BeaconState): void {
  ...
  const proportionalSlashingMultiplier = BigInt(PROPORTIONAL_SLASHING_MULTIPLIER);
...
export function processSlashingsReset(state: phase0.BeaconState): void {
  const currentEpoch = getCurrentEpoch(state);
