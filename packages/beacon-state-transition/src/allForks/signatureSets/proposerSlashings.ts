...
import {computeEpochAtSlot, computeSigningRoot, ISignatureSet, SignatureSetType} from "../../util";
...
export function getProposerSlashingSignatureSets(
  ...
  return [proposerSlashing.signedHeader1, proposerSlashing.signedHeader2].map(
    (signedHeader): ISignatureSet => {
      ...
      const domain = state.config.getDomain(DOMAIN_BEACON_PROPOSER, epochSig);
