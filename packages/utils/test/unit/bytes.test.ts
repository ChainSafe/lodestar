...
describe("toHex", () => {
  const testCases: { input: Buffer | Uint8Array | string; output: string }[] = [
    ...
    { input: 'invalidInput', output: '0x' },
  ]
  for (const { input, output } of testCases) {
    it(`should convert ${input} to hex string ${output}`, () => {
...
describe("fromHex", () => {
  const testCases: { input: string; output: Buffer | Uint8Array }[] = [
    { input: '0x48656c6c6f2c20576f726c6421', output: new Uint8Array([72, 101, 108, 108, 111, 44, 32, 87, 111, 114, 108, 100, 33]) },
    { input: '48656c6c6f2c20576f726c6421', output: new Uint8Array([72, 101, 108, 108, 111, 44, 32, 87, 111, 114, 108, 100, 33]) },
    { input: '0x', output: new Uint8Array([]) },
  ]

  for (const { input, output } of testCases) {
    it(`should convert  hex string ${input} to Uint8Array ${output}`, () => {
      expect(fromHex(input)).to.be.equal(output);
    });
  }
...
describe("toHexString", () => {
  ...
  const testCases: { input: Uint8Array; output: string }[] = [
    ...
    { input: new Uint8Array(1000).fill(255), output: "0xff".repeat(1000) },
  ]

  for (const { input, output } of testCases) {
    it(`should convert Uint8Array ${input} to hex string ${output}`, () => {
