...
describe("ProportionalBidPolicy", () => {
  ...
  it("offers a share of the payload value", () => {
    ...
    expect(policy.computeValue({slot, payloadValueGwei: 1_000_000, coverableGwei: 10_000_000})).toEqual(900_000);
  });

  it("deducts the fixed cost", () => {
    const policy = new ProportionalBidPolicy({shareBps: 10_000, fixedCostGwei: 100, minValueGwei: 0});
    expect(policy.computeValue({slot, payloadValueGwei: 1_000, coverableGwei: 10_000})).toEqual(900);
  });
  ...
  it("never bids below the minimum value", () => {
    ...
    expect(policy.computeValue({slot, payloadValueGwei: 1_000, coverableGwei: 10_000})).toEqual(800);
  });

  it("never bids below zero", () => {
    const policy = new ProportionalBidPolicy({shareBps: 5000, fixedCostGwei: 1_000, minValueGwei: 0});
    expect(policy.computeValue({slot, payloadValueGwei: 100, coverableGwei: 10_000})).toEqual(0);
  });
  ...
  it("caps at the maximum value", () => {
    ...
    expect(policy.computeValue({slot, payloadValueGwei: 1_000, coverableGwei: 10_000})).toEqual(500);
  });
  ...
  it("declines if the builder cannot cover the value", () => {
    ...
    expect(policy.computeValue({slot, payloadValueGwei: 1_000, coverableGwei: 999})).toBeNull();
  });
