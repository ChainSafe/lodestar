import {GENESIS_EPOCH} from "../../constants";
import {getBlockRoot} from "../../util";
import {IEpochProcess} from "../util/epochProcess";
import {CachedBeaconState} from "../util/cachedBeaconState";

export function processJustificationAndFinalization(cachedState: CachedBeaconState, process: IEpochProcess): void {
  const config = cachedState.config;
  const previousEpoch = process.prevEpoch;
  const currentEpoch = process.currentEpoch;

  // Initial FFG checkpoint values have a `0x00` stub for `root`.
  // Skip FFG updates in the first two epochs to avoid corner cases that might result in modifying this stub.
  if (currentEpoch <= GENESIS_EPOCH + 1) {
    return;
  }

  const oldPreviousJustifiedCheckpoint = cachedState.previousJustifiedCheckpoint;
  const oldCurrentJustifiedCheckpoint = cachedState.currentJustifiedCheckpoint;

  // Process justifications
  cachedState.previousJustifiedCheckpoint = cachedState.currentJustifiedCheckpoint;
  const bits = cachedState.justificationBits;
  for (let i = bits.length - 1; i >= 1; i--) {
    bits[i] = bits[i - 1];
  }
  bits[0] = false;

  if (process.prevEpochUnslashedStake.targetStake * BigInt(3) >= process.totalActiveStake * BigInt(2)) {
    cachedState.currentJustifiedCheckpoint = {
      epoch: previousEpoch,
      root: getBlockRoot(config, cachedState, previousEpoch),
    };
    bits[1] = true;
  }
  if (process.currEpochUnslashedTargetStake * BigInt(3) >= process.totalActiveStake * BigInt(2)) {
    cachedState.currentJustifiedCheckpoint = {
      epoch: currentEpoch,
      root: getBlockRoot(config, cachedState, currentEpoch),
    };
    bits[0] = true;
  }
  cachedState.justificationBits = bits;

  // Process finalizations
  // The 2nd/3rd/4th most recent epochs are all justified, the 2nd using the 4th as source
  if (bits[1] && bits[2] && bits[3] && oldPreviousJustifiedCheckpoint.epoch + 3 === currentEpoch) {
    cachedState.finalizedCheckpoint = oldPreviousJustifiedCheckpoint;
  }
  // The 2nd/3rd most recent epochs are both justified, the 2nd using the 3rd as source
  if (bits[1] && bits[2] && oldPreviousJustifiedCheckpoint.epoch + 2 === currentEpoch) {
    cachedState.finalizedCheckpoint = oldPreviousJustifiedCheckpoint;
  }
  // The 1st/2nd/3rd most recent epochs are all justified, the 1st using the 3rd as source
  if (bits[0] && bits[1] && bits[2] && oldCurrentJustifiedCheckpoint.epoch + 2 === currentEpoch) {
    cachedState.finalizedCheckpoint = oldCurrentJustifiedCheckpoint;
  }
  // The 1st/2nd most recent epochs are both justified, the 1st using the 2nd as source
  if (bits[0] && bits[1] && oldCurrentJustifiedCheckpoint.epoch + 1 === currentEpoch) {
    cachedState.finalizedCheckpoint = oldCurrentJustifiedCheckpoint;
  }
}
