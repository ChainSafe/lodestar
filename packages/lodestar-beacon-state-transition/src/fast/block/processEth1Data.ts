 ...
 */
export function getNewEth1Data(config: IBeaconConfig, state: BeaconState, newEth1Data: Eth1Data): Eth1Data | null {
  ...
  if (config.types.Eth1Data.equals(state.eth1Data, newEth1Data)) {
  ...
  const sameVotesCount = readOnlyMap(state.eth1DataVotes, (v) => v).filter((e) =>
    config.types.Eth1Data.equals(e, newEth1Data)
  ).length;
