  ...
  *forwardIterateDescendants(blockRoot: RootHex): IterableIterator<ProtoBlock> {
  ...
  protected isBlockTimely(block: BeaconBlock, blockDelaySec: number): boolean {
    const isBeforeLateBlockCutoff =
      blockDelaySec * 1000 < getSlotComponentDurationMs(this.config, "ATTESTATION_DUE_BPS");
  ...
  private isProposingOnTime(secFromSlot: number): boolean {
    const proposerReorgCutoff = getSlotComponentDurationMs(this.config, "PROPOSER_REORG_CUTOFF_BPS");
