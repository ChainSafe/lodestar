 ...
 *
 * Mutates `finalizedPubkey2index` and `finalizedIndex2pubkey`
 *
...
export function syncPubkeys(
  ...
  if (finalizedPubkey2index.size !== finalizedIndex2pubkey.length) {
    throw new Error(
      `Pubkey indices have fallen out of sync: ${finalizedPubkey2index.size} != ${finalizedIndex2pubkey.length}`
    );
  }
  ...
  for (let i = finalizedPubkey2index.size; i < newCount; i++) {
    ...
    finalizedIndex2pubkey.push(bls.PublicKey.fromBytes(pubkey, CoordType.jacobian)); // Optimize for aggregation
  }
}
