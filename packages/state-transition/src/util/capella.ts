...
export function isFullyWithdrawableValidator(fork: ForkSeq, validatorCredential: ValidatorInfo, balance: number, epoch: number) {
...
export function isPartiallyWithdrawableValidator(fork: ForkSeq, validatorCredential: ValidatorInfo, balance: number, epoch: number) {
  ...
  if (fork === ForkSeq.electra) {
    return hasExecutionWithdrawalCredential(withdrawalCredential) && getValidatorExcessBalance(withdrawalCredential, balance) > 0;
  }
