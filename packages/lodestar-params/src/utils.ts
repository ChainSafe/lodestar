...
const beaconParamsType = new ContainerType({
  fields: {
    ...
    // Deposit contract
    DEPOSIT_CONTRACT_ADDRESS: new ByteVectorType({length: 32}),
    
    // Gwei Values
    MIN_DEPOSIT_AMOUNT: new BigIntUintType({byteLength: 8}),
    MAX_EFFECTIVE_BALANCE: new BigIntUintType({byteLength: 8}),
    EJECTION_BALANCE: new BigIntUintType({byteLength: 8}),
    EFFECTIVE_BALANCE_INCREMENT: new BigIntUintType({byteLength: 8}),

    // Initial values
    SAFE_SLOTS_TO_UPDATE_JUSTIFIED: new NumberUintType({byteLength: 8}),
    BLS_WITHDRAWAL_PREFIX: new BitVectorType({length: 8}),
    GENESIS_FORK_VERSION: new ByteVectorType({length: 4}),
...
export function createIBeaconParams(input: Record<string, unknown>): Partial<IBeaconParams> {
  ...
  Object.entries(beaconParamsType.fields).forEach(([fieldName, fieldType]) => {
    if (input[fieldName]) {
      console.log('input: ', input[fieldName]);
