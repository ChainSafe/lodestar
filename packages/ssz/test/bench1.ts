...
interface PendingAttestation1 {
  aggregationBits: BitList;
  inclusionDelay: number;
  proposerIndex: number;
}

interface PendingAttestation2 {
  aggregationBits: ArrayLike<boolean>;
  inclusionDelay: number;
  proposerIndex: number;
}
const pendingAttestationType1 = ssz.parseType({
  fields: [
    ["aggregationBits", {elementType: "bool", maxLength: 2 ** 10}],
    ["inclusionDelay", "number64"],
    ["proposerIndex", "number64"],
  ],
});

const bitListType = new types.BitListType({limit: 2 ** 10});

const pendingAttestationType2 = new types.ContainerType<PendingAttestation2>({
  fields: [
    ["aggregationBits", bitListType],
    ["inclusionDelay", number64Type],
    ["proposerIndex", number64Type],
  ]
})
...
//console.log(ssz.serialize(validatorRegistryType1, r1));
//console.log(r2.size())
...
const pa1 = {
  aggregationBits: BitList.fromBitfield(Buffer.from([3]), 3),
  inclusionDelay: 100,
  proposerIndex: 50,
}

const pa2 = {
  aggregationBits: [true, true, false],
  inclusionDelay: 100,
  proposerIndex: 50,
};

console.log(Buffer.from(pendingAttestationType2.serialize(pa2)));
console.log(ssz.serialize(pendingAttestationType1, pa1));
console.log('---')
console.log(Buffer.from(pendingAttestationType2.hashTreeRoot(pa2)));
console.log(ssz.hashTreeRoot(pendingAttestationType1, pa1));
