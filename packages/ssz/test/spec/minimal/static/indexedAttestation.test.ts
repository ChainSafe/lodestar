...
import {Hash, IndexedAttestation} from "@chainsafe/eth2.0-types";
...
["ssz_lengthy", "ssz_max", "ssz_nil", "ssz_one", "ssz_random", "ssz_random_chaos", "ssz_zero"].forEach((caseName) => {
  ...
  describeDirectorySpecTest<IBaseSSZStaticTestCase<IndexedAttestation>, IResult>(
    `IndexedAttestation ${caseName} minimal`,
    join(__dirname, `${TEST_CASE_LOCATION}/tests/minimal/phase0/ssz_static/IndexedAttestation/${caseName}`),
    (testcase) => {
      const serialized = serialize(testcase.serialized, config.types.IndexedAttestation);
      const root = hashTreeRoot(testcase.serialized, config.types.IndexedAttestation);
      return {
        serialized,
        root
      };
    },
    {
      ...
      // @ts-ignore
      sszTypes: {
        serialized: config.types.IndexedAttestation,
      },
