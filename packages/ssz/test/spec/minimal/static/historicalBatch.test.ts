...
import {Hash, HistoricalBatch} from "@chainsafe/eth2.0-types";
...
["ssz_lengthy", "ssz_max", "ssz_nil", "ssz_one", "ssz_random", "ssz_random_chaos", "ssz_zero"].forEach((caseName) => {
  ...
  describeDirectorySpecTest<IBaseSSZStaticTestCase<HistoricalBatch>, IResult>(
    `HistoricalBatch ${caseName} minimal`,
    join(__dirname, `${TEST_CASE_LOCATION}/tests/minimal/phase0/ssz_static/HistoricalBatch/${caseName}`),
    (testcase) => {
      const serialized = serialize(testcase.serialized, config.types.HistoricalBatch);
      const root = hashTreeRoot(testcase.serialized, config.types.HistoricalBatch);
      return {
        serialized,
        root
      };
    },
    {
      ...
      // @ts-ignore
      sszTypes: {
        serialized: config.types.HistoricalBatch,
      },
