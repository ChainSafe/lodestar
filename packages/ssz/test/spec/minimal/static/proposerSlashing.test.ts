...
import {Hash, ProposerSlashing} from "@chainsafe/eth2.0-types";
...
["ssz_lengthy", "ssz_max", "ssz_nil", "ssz_one", "ssz_random", "ssz_random_chaos", "ssz_zero"].forEach((caseName) => {
  ...
  describeDirectorySpecTest<IBaseSSZStaticTestCase<ProposerSlashing>, IResult>(
    `ProposerSlashing ${caseName} minimal`,
    join(__dirname, `${TEST_CASE_LOCATION}/tests/minimal/phase0/ssz_static/ProposerSlashing/${caseName}`),
    (testcase) => {
      const serialized = serialize(testcase.serialized, config.types.ProposerSlashing);
      const root = hashTreeRoot(testcase.serialized, config.types.ProposerSlashing);
      return {
        serialized,
        root
      };
    },
    {
      ...
      // @ts-ignore
      sszTypes: {
        serialized: config.types.ProposerSlashing,
      },
