...
import {AttestationDataAndCustodyBit, Hash} from "@chainsafe/eth2.0-types";
...
["ssz_lengthy", "ssz_max", "ssz_nil", "ssz_one", "ssz_random", "ssz_random_chaos", "ssz_zero"].forEach((caseName) => {
  ...
  describeDirectorySpecTest<BaseSSZStaticTestCase<AttestationDataAndCustodyBit>, Result>(
    `attestation data and custody bit ${caseName} minimal`,
    join(__dirname, `../../../../../spec-test-cases/tests/minimal/phase0/ssz_static/AttestationDataAndCustodyBit/${caseName}`),
    (testcase) => {
      const serialized = serialize(testcase.serialized, config.types.AttestationDataAndCustodyBit);
      const root = hashTreeRoot(testcase.serialized, config.types.AttestationDataAndCustodyBit);
      return {
        serialized,
        root
      };
    },
    {
      ...
      // @ts-ignore
      sszTypes: {
        serialized: config.types.AttestationDataAndCustodyBit,
      },
