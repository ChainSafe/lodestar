import {ForkName, ForkPostFulu, ForkPostGloas, ForkPreFulu, ForkPreGloas, NUMBER_OF_COLUMNS} from "@lodestar/params";
import {BeaconBlockBody, ColumnIndex, SignedBeaconBlock, Slot, deneb, fulu, gloas} from "@lodestar/types";
import {prettyBytes, toRootHex, withTimeout} from "@lodestar/utils";
...
export type BlockInput = BlockInputPreData | BlockInputColumns | BlockInputNoData;

export function isBlockInputPreData(blockInput: IBlockInput): blockInput is BlockInputPreData {
...
type BlockInputPreDataState = {
  hasBlock: true;
  hasAllData: true;
  block: SignedBeaconBlock<ForkPreFulu>;
  source: SourceMeta;
  timeCompleteSec: number;
};
