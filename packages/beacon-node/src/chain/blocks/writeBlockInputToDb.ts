...
export async function removeEagerlyPersistedBlockInputs(this: BeaconChain, blockInputs: IBlockInput[]): Promise<void> {
  ...
  // remove block inputs from pending writes
  this.unfinalizedBlockWrites.filterJobs((args) => {
    const pending = args[0].filter((blockInput) => blockInputs.includes(blockInput));
    // if all block inputs are to be removed from a job, remove the entire job
    if (pending.length > 0) {
      args[0] = pending;
      return true;
    }
