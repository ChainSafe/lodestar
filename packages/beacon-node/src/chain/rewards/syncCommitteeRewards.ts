...
export async function computeSyncCommitteeRewards(block: allForks.BeaconBlock,   preState: CachedBeaconStateAllForks, filters?: (ValidatorIndex | string)[]): Promise<SyncCommitteeRewards> {
  ...
  const balances: Map<ValidatorIndex, {val: number}> = new Map(committeeIndices.map(i => [i, {val: preStateAltair.balances.get(i)}]));  // Use val for convenient way to increment/decrement balance
  ...
  if (filters !== undefined) {
    ...
    return rewards.filter(reward => filters.includes(reward.validatorIndex) || filters.includes(index2pubkey[reward.validatorIndex].toHex()));
  } else {
