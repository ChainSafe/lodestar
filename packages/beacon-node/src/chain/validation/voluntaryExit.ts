...
export async function validateApiVoluntaryExit(
  ...
  return validateVoluntaryExit(engine, voluntaryExit, prioritizeBls);
}
...
async function validateVoluntaryExit(
  ...
  if (engine.opPool.hasSeenVoluntaryExit(voluntaryExit.message.validatorIndex)) {
  ...
  const state = await engine.getHeadStateAtCurrentEpoch(RegenCaller.validateGossipVoluntaryExit);
  ...
  const signatureSet = getVoluntaryExitSignatureSet(engine.config, state, voluntaryExit);
