...
import {GossipHandlerParam, GossipHandlerParamGeneric, GossipHandlers, GossipType} from "../gossip/interface.js";
import {
...
export function getGossipHandlers(modules: ValidatorFnsModules, options: GossipHandlerOpts): GossipHandlers {
  ...
  return {
    ...
    [GossipType.blob_sidecar]: async (_gossipHandlerParam: GossipHandlerParam) => {
    ...
    [GossipType.beacon_attestation]: async (
      ...
      const {results: validationResults} = await validateGossipAttestationsSameAttData(
        fork,
        chain,
        validationParams,
        subnet
