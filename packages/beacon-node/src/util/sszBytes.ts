 ...
 *       └─ ExecutionPayload fixed portion includes slotNumber at offset 528
 */
...
// withdrawals_offset(4) + blobGasUsed(8) + excessBlobGas(8) = 528
const SLOT_NUMBER_OFFSET_IN_EXECUTION_PAYLOAD = 528;

// Payload data starts right after the envelope's fixed portion
const ENVELOPE_START_IN_SIGNED =
  SIGNED_EXECUTION_PAYLOAD_ENVELOPE_MESSAGE_OFFSET +
  SIGNED_EXECUTION_PAYLOAD_ENVELOPE_SIGNATURE_SIZE; // 100

const SLOT_OFFSET_IN_SIGNED_EXECUTION_PAYLOAD_ENVELOPE =
  ENVELOPE_START_IN_SIGNED +
  EXECUTION_PAYLOAD_ENVELOPE_FIXED_SIZE +
  SLOT_NUMBER_OFFSET_IN_EXECUTION_PAYLOAD; // 100 + 48 + 528 = 676
