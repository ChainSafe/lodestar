...
export function getDataColumnSidecars(
  ...
  const sidecars = Array.from({length: NUMBER_OF_COLUMNS}, () => ({
    index: 0,
    column: new Array(contents.blobs.length),
    kzgProofs: new Array(contents.blobs.length),
    kzgCommitments: blobKzgCommitments,
    signedBlockHeader,
    kzgCommitmentsInclusionProof,
  })) as electra.DataColumnSidecars;
