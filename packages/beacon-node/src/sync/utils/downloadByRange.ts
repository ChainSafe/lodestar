...
export function cacheByRangeResponses({
  ...
  for (const {blockRoot, columnSidecars} of responses.validatedColumnSidecars ?? []) {
    ...
    // Gloas columns are added to PayloadEnvelopeInput below, not to IBlockInput
...
export async function validateColumnsByRangeResponse(
  ...
  for (const {blockRoot, block} of blocks) {
    ...
    if (!isForkPostFulu(forkName)) {
      const firstData = columnSidecars.at(0);
      const dataSlot = firstData
        ? isGloasDataColumnSidecar(firstData)
          ? firstData.slot
          : firstData.signedBlockHeader.message.slot
        : undefined;
      throw new DownloadByRangeError({
    ...
    const blobCount = kzgCommitments.length;
 ...
 * - Builds a Map<Slot, SignedExecutionPayloadEnvelope> from received envelopes
 ...
 * - Missing envelopes for blocks with bids are allowed (chain processing handles per-block null envelopes)
 */
