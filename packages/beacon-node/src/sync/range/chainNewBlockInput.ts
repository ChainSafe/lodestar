...
export type SyncChainNewBlockInputFns = {
  /**
   * Must return if ALL blocks are processed successfully
   * If SOME blocks are processed must throw BlockProcessorError()
   */
  processChainSegment: (blocks: BlockInputOld[], syncType: RangeSyncType) => Promise<void>;
  downloadBlockInputByRange: (peerIdStr: PeerIdStr) => Promise<BlockInput[]>;
...
export class SyncChain {
  ...
  private async sendBatchNewBlockInput(): Promise<void> {}
    ...
    try {
      ...
      if (!res.err) {
        ...
        const blobs = res.result.reduce((acc, blockInput) => {
          hasPostDenebBlocks ||= blockInput.type === BlockInputTypeOld.availableData;
          return hasPostDenebBlocks
            ? acc + (blockInput.type === BlockInputTypeOld.availableData ? blockInput.blockData.blobs.length : 0)
            : 0;
        }, 0);
