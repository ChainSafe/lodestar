...
// NOTE: How to run
// EL_BINARY_DIR=g11tech/geth:withdrawals EL_SCRIPT_DIR=gethdocker yarn mocha test/sim/4844-interop.test.ts
// ```
...
describe("executionEngine / ExecutionEngineHttp", function () {
  ...
  it("Post-merge, run for a few blocks", async function () {
    ...
    const {elClient, tearDownCallBack} = await runEL(
      {...elSetupConfig, mode: ELStartMode.PostMerge, genesisTemplate: "4844Post.tmpl"},
  ...
  async function runNodeWithEL(
    this: Context,
    {elClient, eip4844Epoch, testName}: {elClient: ELClient; eip4844Epoch: Epoch; testName: string}
  ): Promise<void> {
    ...
    // Should just try till epoch 2 sufficient time to include a blob tx
    const expectedEpochsToFinish = 2;
