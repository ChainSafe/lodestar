...
describe("network / metadata / getENRForkID", function () {
  it("should get enr fork id if not found next fork", () => {
    ...
    expect(ssz.Version.equals(enrForkID.nextForkVersion, Buffer.from([255, 255, 255, 255]))).to.be.true;
    expect(enrForkID.nextForkEpoch).to.be.eql(Number.MAX_SAFE_INTEGER);
