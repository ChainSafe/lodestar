...
describe("ValidatorMonitor", () => {
  ...
  describe("onceEveryEndOfEpoch pruning", () => {
    ...
    it("should log validator changes at epoch end", () => {
      ...
      // Call onceEveryEndOfEpoch
      monitor.onceEveryEndOfEpoch(headState);

      // Check that "Validator monitor status" was logged
      expect(logSpy).toHaveBeenCalledWith(
        "Validator monitor status",
  ...
  describe("epoch-based logging with prettyPrintIndices", () => {
    it("should format indices using prettyPrintIndices in log", () => {
      const logSpy = vi.spyOn(logger, "info");
      const monitor = createValidatorMonitor(null, config, genesisTime, logger, {});

      // Register sequential validators
      monitor.registerLocalValidator(1);
      monitor.registerLocalValidator(2);
      monitor.registerLocalValidator(3);
      monitor.registerLocalValidator(5); // Gap
      monitor.registerLocalValidator(10);
      ...
      // The log should contain formatted indices like "[1-3, 5, 10]"
      const call = logSpy.mock.calls.find((c) => c[0] === "Validator monitor status");
      expect(call).toBeDefined();
      if (call) {
        const context = call[1] as {indices: string; added: string};
        // Check that indices are formatted with ranges
        expect(context.indices).toContain("[");
        expect(context.indices).toContain("]");
        // The format should show "1-3" for sequential indices
        expect(context.indices).toMatch(/1-3/);
      }
    });

    it("should not log if no validators were added or removed", () => {
      const logSpy = vi.spyOn(logger, "info");
      const monitor = createValidatorMonitor(null, config, genesisTime, logger, {});
      ...
      // Should not log "Validator monitor status" since no changes
      const call = logSpy.mock.calls.find((c) => c[0] === "Validator monitor status");
      expect(call).toBeUndefined();
    });

    it("should clear tracking sets after logging", () => {
      const logSpy = vi.spyOn(logger, "info");
      const monitor = createValidatorMonitor(null, config, genesisTime, logger, {});
      ...
      // First epoch end - should log
      monitor.onceEveryEndOfEpoch(headState);
      expect(logSpy).toHaveBeenCalledTimes(1);

      // Second epoch end - should NOT log since no new changes
      logSpy.mockClear();
      monitor.onceEveryEndOfEpoch(headState);
