...
describe("lightclient api", function () {
  ...
  it.skip("getLightClientFinalityUpdate()", async function () {
    ...
    expect(await res.value()).toBeDefined();
  });
  ...
  it("getLightClientCommitteeRoot() for the 1st period", async function () {
    ...
    expect(await committeeRes.error()).toBe(null);
    const client = getClient({baseUrl: `http://127.0.0.1:${restPort}`}, {config}).beacon;
    const validatorResponse = await client.getStateValidators({stateId: "head"});
    const pubkeys = (await validatorResponse.value()).map((v) => v.validator.pubkey);
    ...
    expect(await committeeRes.value()).toEqual([
