...
describe("data serialization through worker boundary", () => {
  ...
  const workerApiParameters: {[K in keyof NetworkWorkerApi]: Parameters<NetworkWorkerApi[K]>} = {
    ...
    publishPartialMessage: [{topic: "test-topic", groupID: bytes, partialMessage: bytes, partsMetadata: bytes}],
    publishPartialMessageToPeer: [
      peerId,
      {topic: "test-topic", groupID: bytes, partialMessage: bytes, partsMetadata: bytes},
    ],
    getPartialPeers: ["test-topic"],
    getPeerPartialMetadata: ["test-topic", bytes, peerId],
    reportInvalidPartialMessage: [peerId, "test-topic"],
    reportUsefulPartialMessage: [peerId, "test-topic", bytes],
    close: [],
    scrapeMetrics: [],
    writeProfile: [0, ""],
  ...
  const workerApiReturnType: {[K in keyof NetworkWorkerApi]: Resolves<ReturnType<NetworkWorkerApi[K]>>} = {
    ...
    updateStatus: null,
    publishGossip: 1,
    publishPartialMessage: null,
    publishPartialMessageToPeer: null,
    getPartialPeers: [peerId],
    getPeerPartialMetadata: bytes,
    reportInvalidPartialMessage: null,
    reportUsefulPartialMessage: null,
    close: null,
