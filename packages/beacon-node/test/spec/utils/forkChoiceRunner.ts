...
export const BLOBS_FILE_NAME = "^(blobs)_([0-9a-zA-Z]+)$";
export const COLUMN_FILE_NAME = "^(column)_([0-9a-zA-Z]+)$";
export const EXECUTION_PAYLOAD_ENVELOPE_FILE_NAME = "^(execution_payload_envelope)_([0-9a-zA-Z]+)$";
export const ATTESTATION_FILE_NAME = "^(attestation)_([0-9a-zA-Z])+$";
export const ATTESTER_SLASHING_FILE_NAME = "^(attester_slashing)_([0-9a-zA-Z])+$";
            ...
            // block step
              ...
              try {
                ...
                await chain.processBlock(blockImport, {
                  ...
                  // bls_setting==2 means fixtures use placeholder signatures (per
                  // spec test format). Compliance fork_choice fixtures all use
                  // this; standard fork_choice fixtures use bls_setting==1.
                  validSignatures: testcase.meta?.bls_setting === BigInt(2),
                });
                if (!isValid) throw Error("Expect error since this is a negative test");
              } catch (e) {
                // Compliance fork_choice fixtures intentionally re-import the
                // same block (see `dup_shift` mutations in their meta.yaml).
                // Spec semantics: `on_block(store, block)` for an already-known
                // block is a successful no-op. Lodestar's `processBlock` rejects
                // with ALREADY_KNOWN — correct in production, but for spec tests
                // we treat it as success when the step expects validity.
...
export type SpecTestCheckpoint = {epoch: bigint; root: string};
...
export type OnAttesterSlashing = {
  /**
...
export type OnExecutionPayloadEnvelope = {
...
export type ForkChoiceTestCase = {
  meta?: {
    description?: string;
    bls_setting: bigint;
  };
