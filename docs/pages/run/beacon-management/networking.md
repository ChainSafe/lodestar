...
By default, Lodestar listens on IPv4 and also on IPv6 when the host has a global IPv6 address. To listen only on IPv4 or IPv6, set only `--listenAddress` or `--listenAddress6`, respectively.
