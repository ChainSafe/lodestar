...
const randomRequestId = (): RequestId =>
  Array.from({length: 16}, () => randomNibble()).join('');

const multicodec = '/eth/serenity/beacon/rpc/1';
...
export class RpcController extends EventEmitter {
  ...
  public async dialForRpc(peerInfo: PeerInfo): Promise<void> {
    ...
    try {
      const conn = await promisify(this.libp2p.dialProtocol.bind(this.libp2p))(peerInfo, multicodec);
      const peer = this.addPeer(peerInfo);
  ...
  public async start(): Promise<void> {
    ...
    this.libp2p.handle(multicodec, this.onConnection.bind(this));
